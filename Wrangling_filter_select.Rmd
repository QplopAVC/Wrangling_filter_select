...
There are 1248 movies from 1995.

2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?

```{r}
comedies_filtered <- movies %>%
  filter(Comedy == 1, mpaa != "", length > 120, rating > 8)
comedies_filtered
```

"Big Fish", "Forrest Gump", "Homem Que Copiava, O", "Incredibles, The", are the movies.
...
highest_rated <- harry_potter_movies %>%
  arrange(desc(rating)) %>%
  head(1)

most_voted <- harry_potter_movies %>%
  arrange(desc(votes)) %>%
  head(1)

list(included_movies = harry_potter_movies, highest_rated = highest_rated, most_voted = most_voted)
```

The movies are Harry Potter and the Chamber of Secrets, Harry Potter and the Prisoner of Azkaban, Harry Potter and the Sorcerer's Stone. The highest rated one is "...the Prisoner of Azkaban". The one with the most votes is "... the Socerer's Stone".
